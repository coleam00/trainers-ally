...
<p align="center">
  ...
  to create highly customized workouts for their clients in just minutes by working alongside Artifical Intelligence.
</p>
...
<p align="center">
  ...
  alternatives from the personal trainer. This is first application of its kind - never has there been a way for personal trainers
...
The Vercel hosted frontend can be used to view shared workouts (like by clicking the Trainer's Ally link at the top of the README), but cannot be used currently to generate your own workouts.

Feel free though to run this application locally though with the instructions below and your own API keys and everything will work!
...
6. If you decide to revise the workout, the AI will make the revisions and produce the new workout. Feel free to select altneratives here before moving to the next day.
...
After setting up the .env file, run the below commands to create a Python virtual environment and install the necessary Python packages to run the LangServe endpoint. Note that this requires you already have Python and PIP installed on your system.
...
- Add more safety measures in the backend to prevent long running workout generating sessions.
